    ...
    def convert_to_training_format(self, gold_data: List[Dict[str, Any]], 
        ...
        for item in gold_data:
            try:
                ...
                if 'result_' in str(source_file_path):
                    # 경로에서 result_ 이후 부분 추출
                    parts = str(source_file_path).split('result_', 1)
                    if len(parts) > 1:
                        source_folder = parts[1].replace('\\05_final_result.json', '').replace('/05_final_result.json', '')
                    else:
...
class QwenFineTuner:
    ...
    def setup_model_and_tokenizer(self):
        ...
        try:
            ...
            # gradient checkpointing 비활성화 (4비트 양자화와 충돌 방지)
    ...
    def train(self, train_dataset, val_dataset, output_dir: str = "./qwen3_lora_ttalkkac"):
        ...
        training_args = TrainingArguments(
            ...
            gradient_accumulation_steps=16,  # 적절한 수준으로 감소
            warmup_steps=50,
            learning_rate=2e-4,
            fp16=True,
...
def main():
    ...
    # 파인튜너 초기화
    finetuner = QwenFineTuner("Qwen/Qwen3-8B")
