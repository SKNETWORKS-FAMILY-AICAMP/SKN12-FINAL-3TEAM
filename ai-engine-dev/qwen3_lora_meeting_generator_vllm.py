...
class QwenVLLMMeetingGenerator:
    ...
    def parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("JSON 형식을 찾을 수 없음. 기본 구조로 변환 시도")
            
            # 텍스트에서 정보 추출 시도
            lines = response.split('\n')
            result = {
                "project_name": "회의록 기반 프로젝트",
                "project_purpose": "회의 내용 기반 프로젝트 목적",
                "project_period": "미정",
                "project_manager": "미지정",
                "core_objectives": [],
                "core_idea": "",
            ...
            for line in lines:
                line = line.strip()
                if "프로젝트" in line or "project" in line.lower():
                    if not result["project_name"] == "회의록 기반 프로젝트":
                        result["project_name"] = line[:100]
                elif "목적" in line or "purpose" in line.lower():
                    result["project_purpose"] = line[:200]
                elif "목표" in line or "objective" in line.lower():
                    result["core_objectives"].append(line[:150])
                elif "아이디어" in line or "idea" in line.lower():
                    result["core_idea"] = line[:200]
                elif "계획" in line or "plan" in line.lower():
                    result["execution_plan"] = line[:300]
                elif "효과" in line or "effect" in line.lower():
                    result["expected_effects"].append(line[:150])
            
            # 빈 리스트 처리
            if not result["core_objectives"]:
                result["core_objectives"] = ["목표 1: 회의 내용 구현", "목표 2: 프로젝트 완성", "목표 3: 성과 달성"]
            if not result["expected_effects"]:
                result["expected_effects"] = ["기대효과 1: 업무 효율성 향상", "기대효과 2: 프로세스 개선", "기대효과 3: 성과 창출"]
            
            # 빈 문자열 처리
            if not result["core_idea"]:
                result["core_idea"] = response[:200] if response else "회의 내용 기반 아이디어"
            if not result["idea_description"]:
                result["idea_description"] = response[:300] if response else "회의에서 논의된 내용을 기반으로 한 프로젝트"
            if not result["execution_plan"]:
                result["execution_plan"] = "1단계: 요구사항 분석\n2단계: 설계 및 개발\n3단계: 테스트 및 배포"
            
            logger.info("기본 구조로 변환 완료")
            return result
            
        except json.JSONDecodeError as e:
            logger.error(f"JSON 파싱 실패: {e}")
            logger.debug(f"파싱 시도한 텍스트: {response[:500]}...")
            
            # 최종 폴백: 기본 구조 반환
            return {
                "project_name": "JSON 파싱 실패 - 회의록 프로젝트",
                "project_purpose": "회의 내용 분석 필요",
                "project_period": "미정",
                "project_manager": "미지정",
                "core_objectives": ["목표 1: 회의 내용 재분석 필요", "목표 2: JSON 형식 확인 필요", "목표 3: 데이터 구조 검증 필요"],
                "core_idea": response[:200] if response else "파싱 실패로 인한 재처리 필요",
                "idea_description": "JSON 파싱 실패로 인해 원본 텍스트 분석 필요",
                "execution_plan": "재처리 필요",
                "expected_effects": ["효과 1: 데이터 구조 개선", "효과 2: 파싱 로직 개선", "효과 3: 안정성 향상"],
                "raw_text": response[:1000] if response else ""
            }
        except Exception as e:
            logger.error(f"예상치 못한 오류: {e}")
            return {
                "project_name": "오류 발생",
                "project_purpose": "오류로 인한 처리 실패",
                "project_period": "미정",
                "project_manager": "미지정",
                "core_objectives": ["재처리 필요"],
                "core_idea": "오류 발생",
                "idea_description": str(e),
                "execution_plan": "재시도 필요",
                "expected_effects": ["재처리 후 확인"],
                "error": str(e)
            }
