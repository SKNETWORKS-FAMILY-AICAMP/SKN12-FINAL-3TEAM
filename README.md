...
## 데이터
1) 수집 대상 & 규모
국회 상임·특별위원회 회의록: 52개 세션 / 약 49,003 발화, 정제 JSONL
→ 공식 회의 발언을 timestamp, timestamp_order, speaker, text로 구조화. 공개 회의록으로 개인정보 민감정보 없음. 

AMI 다자 대화록(한글 번역): 154개 세션 / 89,773 발화, 정제 JSONL
→ 시간 순/화자 구분 완전, LLM 요약·역할분류·흐름분석에 적합. 

ICSI 다자 대화록(한글 번역): 75개 세션 / 94,793 발화, 정제 JSONL
→ 일부 빈 발화 제거 규칙 적용, 나머지 정상 구조화. 

2) 공통 스키마 (정제 후 JSONL)
각 라인은 아래 4개 필드를 갖습니다.
```
{
